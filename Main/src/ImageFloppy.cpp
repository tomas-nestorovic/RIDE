...
using namespace Yahel;
	...
	TStdWinError CFloppyImage::SetMediumTypeAndGeometry(RCFormat format){
				...
				track=scannedTracks.n;
					...
					do{
						...
						if (outSectorIndex=ScanTrack( track, ids, lengths )){
							...
							outChs.head=track&1;
							outChs.sectorId=ids[outSectorIndex];
							return;
						}//else
					...
					switch (revolution){
						case Revolution::ANY_GOOD:
							*pOutDataReady=true; // assumption (all Revolutions already attempted, none holding healthy data)
							for( TRev r=0; r<GetAvailableRevolutionCount(track>>1,track&1); r++ )
