...
formatError: ::SetLastError(ERROR_BAD_FORMAT);
				...
				if (tracks[t] && tracksDebug[t]){ // Track exists and read
					...
					for( BYTE i=0; i<ti->nSectors; i++,dataStart+=__getSectorLength__(si++) )
						if (!sectorsDebug[i].modified // Sector not marked Modified ...
							&&
							sectorsDebug[i].crc16!=Checksum::GetCrcIbm3740( dataStart, __getSectorLength__(si) ) // ... but its data show different CRC
						){
	...
	void CDsk5::GetTrackData(TCylinder cyl,THead head,Revolution::TType rev,PCSectorId bufferId,PCBYTE bufferNumbersOfSectorsToSkip,TSector nSectors,PSectorData *outBufferData,PByteInfo *outByteInfos,PWORD outBufferLengths,TFdcStatus *outFdcStatuses,TLogTime *outDataStarts,TRev *outRevs){
				...
				if (tracksDebug[cyl*diskInfo.nHeads+head]==nullptr){ // Track read for the first time
					TSectorDebug *const sectorsDebug=(TSectorDebug *)::calloc(ti->nSectors,sizeof(TSectorDebug));
						...
						for( BYTE i=0; i<ti->nSectors; i++,dataStart+=__getSectorLength__(si++) ){
							...
							sectorsDebug[i].crc16=Checksum::GetCrcIbm3740( dataStart, __getSectorLength__(si) );
						}
	...
	TStdWinError CDsk5::SetMediumTypeAndGeometry(RCFormat format){
