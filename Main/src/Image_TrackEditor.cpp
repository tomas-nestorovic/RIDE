...
using namespace Charting;
	...
	typedef Time::CSharedColorIntervalArray CRegionArray;
								...
								for( auto it=tr.GetCurrentTimeMetaDataIterator(); it!=tr.GetMetaData().cend(); it++ ){
									...
									if ( continuePainting=p.params.id==id )
										if (it->GetLength()>metaDataLengthMin){
											...
											const Bit::N bitrate=TIME_MILLI(1)/it->GetBitTimeAvg(); // kilobits
						...
						case ID_CHART:{
							...
							const Utils::CSharedPodArray<TLogPoint,TIndex> indexTimes( tr.GetIndexCount() );
								...
								if (indexTimes.length)
									std::swap( *indexTimes.begin(), indexTimes[tr.GetIndexCount()-1] ); // swapping first and last index pulse to disable application of percentiles, unapplicable to Index pulses
