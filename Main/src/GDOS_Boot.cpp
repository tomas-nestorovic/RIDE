...
#include "GDOS.h"
	...
	static constexpr CFormatDialog::TStdFormat StdFormats[]={
		{ _T("DS 80x10"), 0, DefFdMfmFormat512(DD,GDOS_CYLINDERS_COUNT-1,2,GDOS_TRACK_SECTORS_COUNT), 2, 0, 32, 1, GDOS_DIR_FILES_COUNT_MAX }
	};
	...
	TStdWinError CGDOS::__recognizeDisk__(PImage image,TFormat &outFormatBoot){
		...
		if (const TStdWinError err=image->SetMediumTypeAndGeometry( outFormatBoot ))
			return err;
