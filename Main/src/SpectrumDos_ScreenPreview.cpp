#include "stdafx.h"
	...
	LRESULT WINAPI CSpectrumDos::CScreenPreview::__wndProc__(HWND hPreview,UINT uMsg,WPARAM wParam,LPARAM lParam){
		// window procedure
		switch (uMsg){
			case WM_SIZING:{
			...
			case WM_PAINT:{
				// drawing
				PAINTSTRUCT ps;
				const HDC dc=::BeginPaint(hPreview,&ps);
					::GetClientRect(hPreview,&ps.rcPaint);
					if (pSingleInstance->pdt->entry){
						...
						*/
						...
						::FillRect( dc, &ps.rcPaint, CRideBrush::BtnFace );
						::SetBkMode(dc,TRANSPARENT);
						::DrawText( dc, _T("Error: No file to display"),-1, &ps.rcPaint, DT_SINGLELINE|DT_CENTER|DT_VCENTER );
						::LineTo(dc,ps.rcPaint.right,ps.rcPaint.bottom);
						::MoveToEx(dc,ps.rcPaint.right,0,NULL);
						::LineTo(dc,0,ps.rcPaint.bottom);
					}
				::EndPaint(hPreview,&ps);
				return 0;
			}
			case WM_KEYDOWN:
				// character
		...
		return ::DefWindowProc(hPreview,uMsg,wParam,lParam);
	}
	...
	#define STYLE	WS_VISIBLE|WS_CAPTION|WS_SYSMENU|WS_THICKFRAME
	#define LABEL	_T("Screen$")

	CSpectrumDos::CScreenPreview *CSpectrumDos::CScreenPreview::pSingleInstance;

	CSpectrumDos::CScreenPreview::CScreenPreview(const CFileManagerView *pFileManager)
		// ctor
		// - base
		: CFilePreview(	::CreateWindowEx(	WS_EX_TOPMOST,
							WC_STATIC,LABEL, STYLE,
							CW_USEDEFAULT,CW_USEDEFAULT, 1,1,
							0, 0, app.m_hInstance, NULL
						),
						INI_PREVIEW,
						pFileManager,
						SCREEN_WIDTH, SCREEN_HEIGHT
		...
		::SetWindowLong( hPreviewWnd, GWL_WNDPROC, (UINT)__wndProc__ );
		...
		const HDC dc=::GetDC(hPreviewWnd);
			dib.handle=::CreateDIBSection( dc, &dib.bmi, DIB_RGB_COLORS, (PVOID *)&dib.data, 0,0 );
		::ReleaseDC(hPreviewWnd,dc);
		// - showing the first File
		__showNextFile__();
		// - launching the Flashing
		hFlashTimer=(HANDLE)::SetTimer( hPreviewWnd, ID_FLASH, 500, __flash__ );
		__flash__(hPreviewWnd,WM_TIMER,ID_FLASH,0);
		// - information
	...
	CSpectrumDos::CScreenPreview::~CScreenPreview(){
		// dtor
		// - freeing resources
		::KillTimer(hPreviewWnd,ID_FLASH);
	...
	void CSpectrumDos::CScreenPreview::RefreshPreview(){
		...
		::InvalidateRect(hPreviewWnd,NULL,FALSE);
		...
		::SetWindowText( hPreviewWnd, bufCaption );
	}
