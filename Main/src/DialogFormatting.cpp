#include "stdafx.h"
	...
	void CFormatDialog::PreInitDialog(){
		...
		Yahel::Gui::SetDlgItemIntBuddyW( m_hWnd, ID_SIZE, 0, Yahel::Gui::Decimal, false );
		Yahel::Gui::SetDlgItemIntBuddyW( m_hWnd, ID_GAP, 0, Yahel::Gui::Decimal, false );
		Yahel::Gui::SetDlgItemIntBuddyW( m_hWnd, ID_DIRECTORY, 0, Yahel::Gui::Decimal, false );
	...
	void CFormatDialog::DoDataExchange(CDataExchange *pDX){
		...
		DDX_Text( pDX,	ID_HEAD	,params.format.nHeads );
			const THead nHeadsMax =	params.cylinder0 ? std::min(dos->formatBoot.sides.length,propMedium->nHeadsMax) : propMedium->nHeadsMax;
		...
		if (pDX->m_bSaveAndValidate){
			...
			const Utils::CVarTempReset<TCylinder> nCyls0(
				...
				std::max( params.format.nCylinders+1, dos->formatBoot*dos->formatBoot.nCylinders ) // can only grow (aka. don't modify Format if reformatting Cylinders in the middle of a disk)
			);
	...
	afx_msg void CFormatDialog::__toggleReportingOnFormatting__(){
		...
		const bool verifyTracks=IsDlgItemChecked(ID_VERIFY_TRACK);
