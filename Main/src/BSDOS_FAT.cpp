	...
	bool CBSDOS308::GetSectorStatuses(TCylinder cyl,THead head,TSector nSectors,PCSectorId bufferId,PSectorStatus buffer) const{
		...
		if (const PBootSector bootSector=boot.GetSectorData())
			for( const TLogSector logSectorBase=formatBoot.GetSectorCount(cyl,head)-BSDOS_SECTOR_NUMBER_FIRST; nSectors--; bufferId++ ){
				...
				if (cyl>=formatBoot.nCylinders || head>=formatBoot.sides.length || bufferId->cylinder!=cyl || bufferId->side!=formatBoot.sides[head] || secNum>formatBoot.nSectors || !secNum || bufferId->lengthCode!=BSDOS_SECTOR_LENGTH_STD_CODE) // condition for Sector must be ">", not ">=" (Sectors numbered from 1 - see also "|!id")
