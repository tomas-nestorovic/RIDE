...
#include "HFE.h"
	...
	CString CHFE::UHeader::ListUnsupportedFeatures() const{
		...
		for( ; chs.head<ARRAYSIZE(alternative); chs.head++ )
			if (alternative[chs.head].disabled==0){
				TCHAR buf[80];
				::wsprintf( buf, _T("- alternative encoding for %s"), chs.GetTrackIdDesc(ARRAYSIZE(alternative)) );
				report+=buf;
			}
		return report;
	}
...
formatError: ::SetLastError(ERROR_BAD_FORMAT);
	...
	const CImage::CTrackReader &CHFE::ReadTrack(TCylinder cyl,THead head) const{
