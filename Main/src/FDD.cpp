...
error:				switch (const TStdWinError err=::GetLastError()){
	...
	void CFDD::GetTrackData(TCylinder cyl,THead head,Revolution::TType rev,PCSectorId bufferId,PCBYTE bufferNumbersOfSectorsToSkip,TSector nSectors,PSectorData *outBufferData,PByteInfo *outByteInfos,PWORD outBufferLengths,TFdcStatus *outFdcStatuses,TLogTime *outDataStarts,TRev *outRevs){
		...
		if (const PInternalTrack pit=__scanTrack__(cyl,head)){
			...
			for( const TPlanStep *pPlanStep=plan; pPlanStep<planEnd; pPlanStep++ ){
				...
				outRevs[index]=rsi.currentRevolution;
			}
		}else
...
fdrawcmd:				return	::DeviceIoControl( _HANDLE, IOCTL_FD_SET_DATA_RATE, &transferSpeed,1, nullptr,0, &nBytesTransferred, nullptr )
	...
	TStdWinError CFDD::SetMediumTypeAndGeometry(RCFormat format,PCSide sideMap,TSector firstSectorNumber){
		// sets the given MediumType and its geometry; returns Windows standard i/o error
		LOG_ACTION(_T("TStdWinError CFDD::SetMediumTypeAndGeometry"));
		EXCLUSIVELY_LOCK_THIS_IMAGE();
		// - base
		const auto mediaCombination= floppyType!=Medium::UNKNOWN ? floppyType|format.mediumType : Medium::HDD_RAW|format.mediumType; // make sure two bits are always set
		...
		if (floppyType!=Medium::UNKNOWN
			&&
			(floppyType&Medium::FLOPPY_ANY)!=0 // set in base method to "format.mediumType"
		)
...
formatStandardWay:
				...
				if (params.verifyFormattedTracks){
					...
					GetTrackData( cyl, head, Revolution::ANY_GOOD, bufferId, Utils::CByteIdentity(), nSectors, (PSectorData *)dummyBuffer, (PByteInfo *)dummyBuffer, (PWORD)dummyBuffer, statuses, (PLogTime)dummyBuffer, (TRev *)dummyBuffer ); // "DummyBuffer" = throw away any outputs
