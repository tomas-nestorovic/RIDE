...
#include "TRDOS.h"
	...
	static PDos __instantiate__(PImage image,RCFormat formatBoot){
		return new CTRDOS505(image,formatBoot);
	}

	TStdWinError CTRDOS505::__recognizeDisk__(PImage image,TFormat &outFormatBoot){
		...
		if (const TStdWinError err=CTRDOS503::__recognizeDisk__(image,outFormatBoot)) // explicitly calling TR-DOS 5.03 recognition routine, thus bypassing TR-DOS 5.04 recognition routine
			return err;
	...
	CTRDOS505::CTRDOS505(PImage image,RCFormat formatBoot)
		// ctor
		: CTRDOS504( image, formatBoot, &Properties ) {
	}
