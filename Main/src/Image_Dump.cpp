#include "stdafx.h"
	...
	struct TDumpParams sealed{
		...
		const struct TSourceTrackErrors sealed:TWarnings{
			const TCylinder cyl;
			const THead head;
			const Memory::CSharedPodArray<TSourceSectorError,TSector> erroneousSectors;
		...
		TDumpParams(PCDos _dos)
			...
			, sourceSupportsTrackReading( source->WriteTrack(0,0,Track::Invalid)!=ERROR_NOT_SUPPORTED )
	...
	static UINT AFX_CDECL __dump_thread__(PVOID _pCancelableAction){
		...
		p.targetSupportsTrackWriting=dp.target->WriteTrack(0,0,Track::Invalid)!=ERROR_NOT_SUPPORTED;
...
{LOG_TRACK_ACTION(p.chs.cylinder,p.chs.head,_T("scanning source"));
				...
				if (trSrc && dp.fullTrackAnalysis){
					...
					warnings.hasNonformattedArea=peTrack.Contains( Track::Event::NONFORMATTED );
					warnings.hasDataInGaps=peTrack.Contains( Track::Event::DATA_IN_GAP );
					warnings.hasFuzzyData=peTrack.Contains( Track::Event::FUZZY_BAD );
...
{LOG_TRACK_ACTION(p.chs.cylinder,p.chs.head,_T("reading source"));
				dp.source->GetTrackData( p.chs.cylinder, p.chs.head, Revolution::ANY_GOOD, bufferId, sectorIdAndPositionIdentity, nSectors, bufferSectorData, (PByteInfo *)dummyBuffer, bufferLength, bufferFdcStatus, (PLogTime)dummyBuffer, (TRev *)dummyBuffer ); // reading healthy Sectors (unhealthy ones read individually below); "DummyBuffer" = throw away any outputs
...
setDestination:						// : compacting FileName in order to be better displayable on the button
									...
									EnableDlgItem( ID_MEDIUM, dos->image->WriteTrack(0,0,Track::Invalid)==ERROR_NOT_SUPPORTED );
							...
							case MAKELONG(ID_MEDIUM,CBN_SELCHANGE):{
								...
								EnableDlgItem( ID_CODEC, dos->image->WriteTrack(0,0,Track::Invalid)==ERROR_NOT_SUPPORTED );
